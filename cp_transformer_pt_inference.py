...
from cp_transformer_pt import RoFormerSymbolicTransformer, SOS_TOKEN, EOS_TOKEN
        ...
        for time_step, data in enumerate(output):
            ...
            for i in range(0, len(content), 2):
                ...
                if program!=0:
                    ...
                    break
                pitch_duration = pitch_duration - 1
...
def continuation(model, midi_path, prompt_length=37, generation_length=192, temperature=1.0, n_samples=1):
    x = decompress(model, preprocess_midi(midi_path, 8)[0])
    x = x[:, :prompt_length] 
    # print(x, x.shape) # (1, 37, 8)
    # y = [x[:, i, :] for i in range(x.shape[1])] #把37帧分开了 每一帧一个长度为8的
...
if __name__ == '__main__':
    ...
    model.cuda()
    model.eval()
    for i in os.listdir('/home/coder/laopo/StreamMUSE/input/acc'):
        i = os.path.join('/home/coder/laopo/StreamMUSE/input/acc', i)
        continuation(model, i, temperature=1.0, generation_length=192, n_samples=2, prompt_length=37)
