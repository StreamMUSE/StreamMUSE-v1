...
def test_midi_device_input_satisfies_protocol():
    """MidiDeviceInput has read_events() and close() per InputSource."""
    from streammuse.domain.interfaces import InputSource

    # Don't open a real port; just check the class interface
    assert hasattr(MidiDeviceInput, "read_events")
    assert hasattr(MidiDeviceInput, "close")
