...
def _songs() -> list[int]:
    return _parse_int_list(
        "STREAMMUSE_TWO_STAGE_CONSISTENCY_SONGS",
        os.environ.get("STREAMMUSE_CONSISTENCY_SONGS", DEFAULT_SONGS),
    )


def _tempos() -> list[int]:
    return _parse_int_list(
        "STREAMMUSE_TWO_STAGE_CONSISTENCY_TEMPOS",
        os.environ.get("STREAMMUSE_CONSISTENCY_TEMPOS", DEFAULT_TEMPOS),
    )
...
def test_two_stage_realtime_matches_offline(
    ...
    for tempo in _tempos():
        ...
        realtime_prompt_context = event_counter(
            realtime_prompt_events,
            max_tick=PROMPT_LENGTH_TICKS,
        )
