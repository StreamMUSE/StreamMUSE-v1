...
if [[ "${RUN_PUBLIC_CLIENT}" == "1" ]]; then
  ...
  echo "[spark-bench] prompt-continuation late recovery: recover=${LEKAI_PROMPT_CONTINUATION_RECOVER_LATE_EVENTS} bound=${LEKAI_PROMPT_CONTINUATION_BOUND_LATE_RECOVERY} max_ticks=${LEKAI_PROMPT_CONTINUATION_RECOVER_LATE_MAX_TICKS}"
  LEKAI_SERVER_HOST=127.0.0.1 \
