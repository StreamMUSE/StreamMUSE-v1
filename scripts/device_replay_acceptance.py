...
def identity(system_ref=BASE_COMMIT):
    expected = subprocess.check_output(
        ["git", "-C", str(REPO), "rev-parse", "--verify", system_ref + "^{commit}"], text=True).strip()
    diff = subprocess.check_output(["git", "-C", str(REPO), "diff", expected, "--", "src", "transformers"])
    if diff:
    ...
    return {"repo": str(REPO), "host": platform.node(), "python": sys.version,
            ...
            "base": expected, "system_source_unchanged": True,
...
def server(args):
    ...
    os.environ["STREAMMUSE_CODE_SHA"] = identity(args.system_ref)["head"]
...
def clients(args):
    ...
    save(args.out / "contract.json", {"source":str(source), "source_sha256":sha(source.read_bytes()),
        ...
        "server":args.server, "device_run_seed": args.seed if args.seed is not None else "generated by existing session initialize API",
        "replay_seed":"copied from device run", "system_code_unchanged":True,
        "hardware_audio_test":False, "latency_benchmark":False})
    a = one_client(args, "A_virtual_device", source=source, seed=args.seed)
...
def main():
    ...
    parser.add_argument("--system-ref", default=BASE_COMMIT,
                        help="Commit whose src/ and transformers/ must match the tested checkout")
    parser.add_argument("--seed", type=int,
                        help="Optional explicit seed for A; B/C always copy A's recorded seed")
