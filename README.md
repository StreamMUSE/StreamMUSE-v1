...
### Tested realtime defaults

The application/CLI/Web session defaults are playback **90 BPM**, model-conditioning
**80 BPM**, Prompt `rule_s_if_else` with **10 candidates / rank1**, and shared Prompt /
continuation sampling `temperature=1.1`, `top_k=50`, `top_p=0.95`,
`repetition_penalty=1.0`. Model conditioning is independent of playback tempo;
changing the Web tempo does not change the default 80-BPM model condition.
The model time-signature index defaults to **0 (4/4)**. Continuation token context
defaults to **32 beats**, with **128 ticks** of retained model history; cumulative
melody evidence is retained separately. Tonal and EMPTY-token guards default on.

Explicit CLI/session settings still override these defaults. Bare backend sampling
defaults match the application; existing `LEKAI_*` launch overrides still apply
when no explicit session override is supplied. Restart the backend/Web process to
load changed code. Model selection and continuation-mode launch switches are
unchanged: use the existing `--continuation-mode prompt_continuation` launch path.
