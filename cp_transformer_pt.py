...
class RoFormerSymbolicTransformer(L.LightningModule):
    ...
    def global_sampling(self, x, max_seq_len=192, temperature=1.0):
        batch_size, seq_len, subseq_len = x.shape
    ...
    def forward(self, x):
        ...
        # print("after concat sos:", h.shape)
    ...
    def preprocess(self, x, pitch_shift):
        ...
        # print(eos_indices)
        is_not_drum = x[:, :, :, 0] != 127 
        x_processed[:, :, :, 0] = x[:, :, :, 0] # program 不变
        # print(pitch_shift)
        x_processed[:, :, :, 1] = x[:, :, :, 1] + (x[:, :, :, 2]) * 128 + 1 + pitch_shift[:, None, None] * is_not_drum
...
if __name__ == '__main__':
    ...
    model_name = f'cp_transformer_acc_withprog_cp8_{model_size}_batch_{batch_size * n_gpus}_schedule'
    ...
    train_set_loader = DataLoader(FramedDataset('data/com_cp4_v2_acc.pt', TRAIN_LENGTH, batch_size, split='train'), batch_size=None, num_workers=1, persistent_workers=True)
    val_set_loader = DataLoader(FramedDataset('data/com_cp4_v2_acc.pt', TRAIN_LENGTH, batch_size, split='val'), batch_size=None, num_workers=1, persistent_workers=True)
