...
def continuation(model, midi_path, output_path=None, prompt_length=100, generation_length=400, temperature=1.0, n_samples=1, gt_mel=True):
    ...
    for i in range(n_samples):
        ...
        if output_path is None:
            decode_output(output_i, f"temp3/{model.save_name}/prompt{prompt_length}/{os.path.basename(midi_path)}_temp{temperature}_{i}.mid", tempo=90.0)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="../results/ModelBaseline/cp_transformer_909+ac+1k7_trackemb_interleavepos_v0.2_large_batch_40_schedule.epoch=00.val_loss=0.90296.ckpt" ,help="path to model checkpoint")
    parser.add_argument("--prompt_len", type=int, default=200, help="length of prompt")
    ...
    for midi in os.listdir('./input/mel'):
        if midi.endswith('mid'):
            ...
            continuation(model, midi, temperature=args.temperature, generation_length=2, n_samples=args.n_samples, prompt_length=args.prompt_len, gt_mel=True)
