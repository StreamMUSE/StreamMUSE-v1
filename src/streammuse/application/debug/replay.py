...
def _load_prompt_continuation_backend_cls() -> type[Any]:
    try:
        ...
        raise RuntimeError(
            "lekai-prompt-continuation replay requires the prompt-continuation "
            "backend package to be present on this branch"
        ) from exc
    ...
    *,
    ...
    try:
        with _runtime_env(config, stage2_log_dir=stage2_log_dir):
            backend_cls = _load_prompt_continuation_backend_cls()
