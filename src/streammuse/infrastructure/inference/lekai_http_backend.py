...
DEFAULT_PROMPT_CONTEXT_BEATS = 32
...
class LekaiHttpBackend:
    ...
    def _sampling_config(self) -> Dict[str, float | int]:
        ...
        sampling: Dict[str, float | int] = {
            "temperature": _float("LEKAI_RT_TEMPERATURE", 1.1),
            "top_k": _int("LEKAI_RT_TOP_K", 50),
            "top_p": _float("LEKAI_RT_TOP_P", 0.95),
            "repetition_penalty": _float("LEKAI_RT_REPETITION_PENALTY", 1.0),
        }
    ...
    def _warmup_model(self, warmup_steps: int) -> float:
        ...
        prompt = torch.tensor(
            self._continuation_initial_tokens(
                bpm=int(os.environ.get("LEKAI_DEFAULT_BPM", "80")),
                time_signature_idx=int(
                    os.environ.get("LEKAI_TIME_SIGNATURE_INDEX", "0")
                ),
            )
    ...
    def runtime_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "effective_bpm": (
                ...
                else int(os.environ.get("LEKAI_DEFAULT_BPM", "80"))
            ),
            **sampling,
            ...
            "time_signature_index": int(os.environ.get("LEKAI_TIME_SIGNATURE_INDEX", "0")),
    ...
    def _generate_with_interleaved_prompt(
        ...
        effective_bpm = self._request_bpm if self._request_bpm is not None else int(os.environ.get("LEKAI_DEFAULT_BPM", "80"))
        # Training metadata uses category 0 for 4/4; 4 is the remapped 2/2 class.
        time_signature_idx = int(os.environ.get("LEKAI_TIME_SIGNATURE_INDEX", "0"))
        ...
        with self._session_gate:
            ...
            metadata: Dict[str, Any] = {
                ...
                "effective_bpm": int(
                    self._request_bpm
                    if self._request_bpm is not None
                    else os.environ.get("LEKAI_DEFAULT_BPM", "80")
                ),
                "generation_start_tick": int(generation_start_tick),
