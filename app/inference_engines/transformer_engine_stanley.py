...
def note_list_to_pretty_midi(notes, tempo=90, program=0, name="track"):
    ...
    for note in notes:
        ...
        midi_note = pretty_midi.Note(velocity=100, pitch=note["pitch"], start=start, end=end)
...
class InferenceEngineStanley:
    ...
    def generate_accompaniment(
        ...
        prompt_melody = [n for n in self.melody_history if prompt_start_tick <= n["tick"] < prompt_end_tick]
        prompt_acc = [n for n in self.accompaniment_history if prompt_start_tick <= n["tick"] < prompt_end_tick]
